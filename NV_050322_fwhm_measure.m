function[fwhm,peak_area]=NV_050322_fwhm_measure(data_fwhm,peak_height)
...
%         end
        ...
        startpt=find(data_fwhm(1:k-1)<=0 & data_fwhm(2:k) >0,1,'last');
        endpt=find(data_fwhm(k:end-1)>0 & data_fwhm(k+1:end) <= 0,1,'last');
        while k1 >= startpt
