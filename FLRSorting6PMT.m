function successmessage=FLRSorting6PMT(filepath,file_range)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% file:FLRSorting6PMT.m
% ***Description***:
% This function serves automatically search for fluorescent peaks
...
% Date Modified: 06/30/2023
% Latest Revision:^PMT modification
% Modifying Author:Taras Hanulia
% Date Modified: 05/17/2024
% sorting for NoScat data. the orijinal function was for Beads sorting
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Function details
% Inputs:
...
for i=file_range
    ...
    if~isempty(subdirinfo{i})
        ...
        for j=min(chunks):max(chunks)
            ...
            a=pv1(:,7)==j;
            loc=pv1(a,8);
            load([subdirinfo{i}(1).name(1:end-9),'NoScat.mat'],'peak_values')
            pv2=peak_values;
            a2=find(pv2(:,7)==j);
            loc2=pv2(a2,8);
            wid2=pv2(a2,10);
        ...
        save([subdirinfo{i}(1).name(1:end-8),'NoSNoFLR.mat'],'peak_values')
        peak_values=del;
        save([subdirinfo{i}(1).name(1:end-8),'NoSFLR.mat'],'peak_values')
    end 
end
...
for i=file_range
    ...
    if~isempty(subdirinfo{i}(1))
        cd(subdirinfo{i}(1).folder)
        load([subdirinfo{i}(1).name(1:end-8),'NoSNoFLR.mat'],'peak_values')
        pv1=[pv1;peak_values]; %#ok<AGROW> 
        load([subdirinfo{i}(1).name(1:end-8),'NoSFLR.mat'],'peak_values')
        pv2=[pv2;peak_values]; %#ok<AGROW> 
    end
end
cd(mainFolder)
 peak_values=pv1;
 save([subdirinfo{i}(1).name(5:end-8),'NoSNoFLR.mat'],'peak_values')
 peak_values=pv2;
 save([subdirinfo{i}(1).name(5:end-8),'NoSFLR.mat'],'peak_values')
successmessage='Completed Seperating FLR Peaks';
