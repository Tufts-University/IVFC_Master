function successmessage = PeakSortingWithFLRTH(filepath, file_range, bead_flag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for K = 1:length(dirinfo)
    ...
    subdirinfo{K} = dir(fullfile(thisdir, '*NTH*NoFLR.mat'));
end
...
for i = file_range
    ...
    if ~isempty(subdirinfo{i})
        ...
        for j = min(chunks):max(chunks)
            ...
            a = find(pv1(:, 6) == j);%a = pv1(:, 7) == j;
            loc1 = pv1(a, 7);
            start1 = pv1(a, 10);
            end1 = pv1(a, 11);
            a2 = pv2(:, 6) == j;%  a2 = find(pv2(:, 7) == j);
            loc2 = pv2(a2, 7);
            start2 = pv2(a2, 10);
            end2 = pv2(a2, 11);
