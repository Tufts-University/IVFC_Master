function successmessage = PeakSortingWithScat(filepath, file_range, bead_flag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% file: PeakSortingWithScat.m
% ***Description***:
...
for i = file_range
    ...
    peaksWithScat = []; % Matrix of peaks with Scat
    peaksWithoutScat = []; % Matrix of peaks without Scat
    ...
    if ~isempty(subdirinfo{i})
        ...
        for j = min(chunks):max(chunks)
            ...
            for k = 1:length(loc2)  
                ...
                match = find(loc1 >= start2(k) & loc1 <= end2(k), 1);   
                if ~isempty(match)
                    peaksWithScat = [peaksWithScat; pv2(a2(k), :)]; %#ok<AGROW> 
                else
                    peaksWithoutScat = [peaksWithoutScat; pv2(a2(k), :)]; %#ok<AGROW>
                end
            end
        end
        
        % Save sorted peaks
        peak_values = peaksWithScat;
        save([subdirinfo{i}(1).name(1:end-9), 'NoScatWithScat.mat'], 'peak_values')
        peak_values = peaksWithoutScat;
        save([subdirinfo{i}(1).name(1:end-9), 'NoScatWithoutScat.mat'], 'peak_values')
    end 
end
...
for i = file_range
    ...
    if ~isempty(subdirinfo{i}(1))
        cd(subdirinfo{i}(1).folder)
        load([subdirinfo{i}(1).name(1:end-9), 'NoScatWithScat.mat'], 'peak_values')
        pv1 = [pv1; peak_values]; %#ok<AGROW>
        load([subdirinfo{i}(1).name(1:end-9), 'NoScatWithoutScat.mat'], 'peak_values')
        pv2 = [pv2; peak_values]; %#ok<AGROW>
    end
end
cd(mainFolder)
peak_values = pv1;
save([subdirinfo{file_range(1)}(1).name(4:end-9), 'NoScatWithScat.mat'], 'peak_values')
peak_values = pv2;
save([subdirinfo{file_range(1)}(1).name(4:end-9), 'NoScatWithoutScat.mat'], 'peak_values')

successmessage = 'Completed Sorting Peaks With and Without Scat';
end