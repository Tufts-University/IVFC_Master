function successmessage=SamplePeakDetection_PCA_PN_TH(filepath,outputfile,...
...
%                     end
                    ...
                    %% Set cluster range for interrogation based on closeness
                        ...
                        for m=1:length(peaks)
                            ...
                            start_point = max(check_ranges(m, 1) + 5, 1); % Starting point of the peak
                            end_point = min(check_ranges(m, 2) - 5, size(M_filt, 1)); % Ending point of the peak, ensuring it does not exceed matrix size
...
%             peak_values(:,19)=all_fwhm_fl2; peak_values(:,20)=all_peak_area_fl2;
            peak_values(:,12)=all_file_num;
            peak_values(:,10)=all_peak_start;%location peak start within chunk
            peak_values(:,11)=all_peak_end;%location peak end within chunk
            peak_values(1,:)=[];
...
%         peak_values(:,19)=all_fwhm_fl2_Store; peak_values(:,20)=all_peak_area_fl2_Store;
        peak_values(:,12)=all_file_num_Store;
        peak_values(:,10)=all_peak_start_Store;%location peak start within chunk
        peak_values(:,11)=all_peak_end_Store;%location peak end within chunk 
        % Sorting peaks
        peak_values=sortrows(peak_values,6);
        delrow=peak_values(:,12)==0;
        peak_values(delrow,:)=[];
...
if bead_flag==1
    successmessage=BeadSortingTH(filepath,file_range);
    disp(successmessage)
end
successmessage=PeakSortingWithFLRTH(filepath, file_range, bead_flag);
disp(successmessage)
