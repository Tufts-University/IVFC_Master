function successmessage=SamplePeakDetection6PMTRF_PCA_PN(filepath,outputfile,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% file:SamplePeakDetection6PMTRF_PCA_PN.m
% ***Description***:
...
% Date Written: 15/05/2023 /11/30/2023 modification 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Function details
% Inputs:
...
% Fs=60e3; %60,000 samples p/er second
...
if isempty(exp_num)
    exp_num=6000/9; % standard 6000/9
end
...
[b,a]=butter(2,Wn);
types={'Scattering Only';'FLR Only';'Cumulative';'iRFP';'E2C';'RFLR'};
        ...
        if~isempty(subdirinfo{i})
            ...
            for ii=1:num_chunks
                ...
                if isempty(M)
                    ...
                    [~,~,~,tsquared,~,~] = pca(M2(:,1:3));
% %                     Norm=1;
%                     SN_Green=SN_Green./11;% 11 or max , usually norm 
%                     SN_Red1=SN_Red1./11;% 11or max , usually norm 
%                     SN_Red2=SN_Red2./11;% 3 or max , usually norm 
                    
                    M2(:,4:6) = [SN_Red1,SN_Green,SN_Red2]./signal_max;
                    norm = tsquared;
...
%                     end
                    ...
                    for m=1:length(peaks)
                        ...
                        if clusterpts(m,2)+10>5400000
                            ...
                            data_fwhm_cum=scat_norm(clusterpts(m,1)-10:clusterpts(m,2)+10);
                        end
                    ...
                    if isempty(idx)==0
                        if f==6||f==2 % f=6 when red fluorescence 6
                            idx2=find(fwhm(idx)>FWMH_threshold  & peak_data(idx,5)>intensity_threshold); % 5 for green fluorescence 
                            if isempty(idx2)==0
