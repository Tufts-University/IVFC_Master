function [finalSens, finalPur] = ScatvFLRCalculator_DeepPeak_ThresholdTest(mainFolder,header)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  file: ScatvFLRCalculator_DeepPeak.m
%  authors: Nilay Vora
%  created: 06/04/2023
% modified:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
datasheet_path = 'T:\Nilay\IVFC\Acquired Data\Blood Cell Data';
cd(mainFolder)
T = readtable([datasheet_path,'\NV_20230328_DatasetSummary.xlsx']);
    ...
    for j=1:length(subdirinfo2)
        ...
        if length(files)>1
            for k=1:length(files)
                if contains(files(k).name,'NEW_peak_values') && contains(files(k).name,search_tag)
            ...
            if isempty(base_name) == 1
                for k=1:length(files)
                    if contains(files(k).name,'NEW_peak_values') && contains(files(k).name,'_NoScat')
                        base_name=files(k).name;
                        break
                    end
                end
...
writetable(T_final,'SensitivityTable_Threshold.xlsx')
